 ...
 * 2026-08-01 linzhiwei 事件驱动：timer_arm/stop + 线程内轮询触发   v1.2.0
 * 2026-07-30 linzhiwei 小缓存 reinit、透传 sink、hs_cmd           v1.1.0
...
static int slave_rt_apply(const slave_rt_cfg_t *c)
{
    ...
    if (hs_cmd_register() != LW_AT_ERR_OK) {
        lw_at_deinit();
        return -1;
    }
    hs_cmd_reset_state();
